...
class Repo extends AbstractApi {
	...
	public function find( $keyword, array $params = array() ) {
		return $this->get(
			'/legacy/repos/search/' . rawurlencode( $keyword ), array_merge(
				array(
					'start_page' => 1,
				), $params
			)
	...
	public function org( $organization, array $params = array() ) {
		return $this->get(
			'/orgs/' . $organization . '/repos', array_merge(
				array(
					'start_page' => 1,
				), $params
			)
	...
	public function readme( $username, $repository, $format = 'raw' ) {
		return $this->get(
			'/repos/' . rawurlencode( $username ) . '/' . rawurlencode( $repository ) . '/readme', [], [
	...
	public function contributors( $username, $repository, $includingAnonymous = false ) {
		return $this->get(
			'/repos/' . rawurlencode( $username ) . '/' . rawurlencode( $repository ) . '/contributors', array(
	...
	public function watchers( $username, $repository, $page = 1 ) {
		return $this->get(
			'/repos/' . rawurlencode( $username ) . '/' . rawurlencode( $repository ) . '/watchers', array(
				'page' => $page,
			)
		);
	}
	...
	public function subscribers( $username, $repository, $page = 1 ) {
		return $this->get(
			'/repos/' . rawurlencode( $username ) . '/' . rawurlencode( $repository ) . '/subscribers', array(
				'page' => $page,
			)
		);
	}
	...
	public function events( $username, $repository, $page = 1 ) {
		return $this->get(
			'/repos/' . rawurldecode( $username ) . '/' . rawurldecode( $repository ) . '/events', [
				'page' => $page,
			]
		);
	}
